...
target "angie-rocky-metadata" {
  ...
  tags = [
    "docker.io/${replace(GITHUB_REPOSITORY, "docker-", "")}:1.10-rocky",
    "ghcr.io/${replace(GITHUB_REPOSITORY, "docker-", "")}:1.10-rocky",
    "docker.io/${replace(GITHUB_REPOSITORY, "docker-", "")}:1.10.3-rocky",
    "ghcr.io/${replace(GITHUB_REPOSITORY, "docker-", "")}:1.10.3-rocky"
  ]
}
